...
| `/team-init global\|local [--force]` | Scaffold `agents-team.json` with every built-in role stamped in place, plus the current `schemaVersion` + `scaffoldVersion` markers and top-level safety defaults like `allowExternalPathScopes: false`. Refuses existing files without `--force`; on `--force` the previous file is copied (not renamed — original stays put until the new write succeeds) to `YYYY-MM-DD-HHMMSS-agents-team.json` first. |
