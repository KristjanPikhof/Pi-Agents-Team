...
|---|---|
| `/team` | Open the keyboard-first dashboard overlay. Tabs are **Workers / Inspect / Console / Cost**; jump with `1`–`4`, or cycle with `tab` / `shift+tab`. The action bar runs `[s]teer [m]sg [n]ew [c]lose [x]cancel [p]rune [r]efresh [y]copy [q]uit` for the selected worker. Console live-tails the worker's assistant text from a bounded ring buffer (`PgUp` pauses follow, `End` resumes). Cost shows per-worker usage for currently tracked workers, plus `Σ` totals for active workers and usage retained from pruned terminal workers. Pruning clears old rows without resetting team token/cost totals. `/team <worker-id>` opens that worker's Inspect tab. The header shows `solo` when routing is off, and idle workers show `[reuse]`. |
