...
Worker RPC events get normalized into compact state: status, last tool, last summary, pending relay questions, token usage, known context budget, and a bounded in-memory Activity stream. Console shows the Activity stream by default as bullet action blocks such as `• Ran npm run typecheck`, with nested output snippets and explicit elision like `… +14 lines hidden`; Raw remains available for debugging. Inspect summarizes the same source under `Recent activity`. Copy payloads put `## Activity` before `## Console timeline (Raw)`. Raw transcripts/events are not persisted or used as a synthesis fallback.
