...
| `/team-result <id>` | Print the compact summary (headline, files, risks, usage) plus the verbatim `<final_answer>` block. The authoritative worker deliverable. |
...
| `/team-init [global\|local] [--force]` | Scaffold `agents-team.json` with every built-in role stamped in place, plus the current `schemaVersion` + `scaffoldVersion` markers, the default `routingMode: "team"`, and top-level worker access defaults like `allowPathsOutsideProject: true`. Refuses existing files without `--force`; on `--force` the previous file is backed up first. |
...
Tip: when configuring roles, omit `thinkingLevel` to inherit the orchestrator's live Pi thinking level after any built-in role default. Set it only when a role needs a fixed level.
