...
| `/team-off [--persist global\|local]` | Flip routing to **solo**: the orchestrator answers directly and `delegate_task` rejects with `Team routing off. Run /team on to delegate.`. Other `agent_*` tools stay live so you can still inspect or steer workers spawned earlier. The widget collapses to a single `Pi Agents Team — solo` line while workers are tracked, and disappears entirely when none are. **Auto-persists** `routingMode: "solo"` to the active `agents-team.json` (the winning layer if one is loaded, otherwise a fresh local stub at `./.pi/agent/agents-team.json`). Override scope with `--persist global\|local`. |
| `/team-on [--persist global\|local]` | Flip routing back to **team**. Errors with an "enable first" hint when `enabled: false`. **Auto-persists** `routingMode: "team"` to the active `agents-team.json` (winning layer or a fresh local stub). Override scope with `--persist global\|local`. |
