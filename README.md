...
Runtime churn and timestamp-only refreshes add no records. A terminal transition, a substantive terminal summary or usage revision, and a prune transition do. Each compact record is at most 16 KiB of serialized UTF-8, with Unicode-safe trimming. Reload restores the active branch only, reads legacy v1 state, and marks restored live or reusable workers `exited` because their RPC processes are not attached. Saved summaries and authoritative usage remain available.
