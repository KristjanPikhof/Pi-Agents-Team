...
|---|---|
| `/team` | Open the keyboard-first dashboard overlay. Tabs are **Workers / Inspect / Console / Cost**; jump with `1`–`4`, or cycle with `tab` / `shift+tab`. The action bar runs `[s]teer [m]sg [n]ew [c]lose [x]cancel [p]rune [r]efresh [y]copy [q]uit` for the selected worker. Inspect and Console support follow mode with `f` (`G` jumps bottom + follows, `g` jumps top); Mac-friendly paging uses `space` / `b` in addition to `PgDn` / `PgUp`. Cost shows per-worker usage for currently tracked workers, plus `Σ` totals for active workers and usage retained from pruned terminal workers. Pruning clears old rows without resetting team token/cost totals. `/team <worker-id>` opens that worker's Inspect tab. The header shows `solo` when routing is off, and idle workers show `[reuse]`. |
