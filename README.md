...
| `/team-result <id>` | Print the compact summary with friendly operator labels (`Worker`, `Status`, `Read files`, `Changed files`, `Risks`, `Usage`) plus the verbatim `<final_answer>` block. Usage token counts may be compacted with `k` for thousands (1,000) and `m` for millions (1,000,000); known context budget appears as `ctx=64%/200k rem=72k`. The authoritative worker deliverable. |
| `/team-enable on\|off [--persist global\|local]` | Flip routing between **team** and **solo** and write `routingMode` to the active `agents-team.json` so the choice survives restart. In solo mode the orchestrator answers directly; `delegate_task` rejects with a routing-off error. Other `agent_*` tools stay live so workers spawned earlier remain reachable. The widget collapses to a single `Pi Agents Team — solo` line while workers are tracked, and disappears entirely when none are; the bottom status line remains `Orchestrator · Working...` or `Orchestrator · Idle`. Pass `--persist global\|local` to force a specific scope; default resolves the winning config layer, falling back to a fresh local stub at `./.pi/agent/agents-team.json`. `/team-enable on` errors with an "enable first" hint when `enabled: false`. |
...
`delegate_task` returns a compact launch receipt: `Created <profile> (<id>)` for fresh agents or `Reusing <profile> (<id>)` for warm reuse, then `Task: ...` and `Next: wait_for_agents workerIds=["<id>"]`.
