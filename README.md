...
| `/team-result <id>` | Print the same compact result surface as `agent_result`: worker title, optional task/status/error, pending relay questions, scan-friendly summary sections when available, and `Result:` followed by the verbatim `<final_answer>` contents (or `No <final_answer> block extracted yet.`). |
