...
- The widget collapses to a single `Pi Agents Team — solo` line when workers are tracked, or hides entirely when none are. The bottom status line stays routing-neutral and only shows `Orchestrator · Working...` or `Orchestrator · Idle`.
