...
- **Persistent action bar.** Single-line footer: `[s]teer [m]sg [n]ew [c]lose [x]cancel [p]rune [r]efresh [y]copy [q]uit` outside Console. In Console, the same `r` slot is contextual and renders as `[r]aw` while Activity is visible or `[r]ctivity` while Raw is visible. Each key dispatches the matching `TeamManager` call against the selected worker. `s`, `m`, and `n` open an inline single-line modal (label + buffer + cursor) above the action bar; `enter` submits, `esc` cancels.
