...
`safety.projectRoot` always has a value at launch time: the project config's root when a project-scope config exists, else `options.cwd`. The launch-policy containment guard (path-scope roots and prompt paths must stay within the project root) therefore always fires — global-only and no-config setups can't accept `pathScopeRoots: ["/"]`. Path containment uses `realpathSync.native` so symlink escapes are caught at both load and delegate time. **`pathScope` is a prompt convention, not an OS sandbox** — Pi's `bash` tool can still execute arbitrary shell commands in the worker's cwd regardless of the declared scope. See CLAUDE.md "Path scope is a prompt convention, not an OS sandbox" for the full framing.
