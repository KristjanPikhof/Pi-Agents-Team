...
- `all_terminal`: every target reached a done/stopped status (`idle`, `completed`, `aborted`, `error`, `exited`). The formatted result starts with `Wait: all_terminal`, reports how many agents finished or stopped, and points to `agent_result` for each worker.
- `relay_raised`: any target raised a new relay question while running. The response carries a `newRelays` list, and the formatted result starts with `Wait: relay_raised`, lists profile/id/question plus an `agent_message` response hint, and says to wait again after answering. Opt out with `wakeOnRelay: false`.
...
- `no_workers`: no tracked workers matched the wait request. The formatted result starts with `Wait: no_workers` and tells the orchestrator to delegate first.
