...
- **Content/event split.** Inspect builds a structured text document from worker state (`buildInspectText`) with explicit section dividers for Status, Recent activity, Task, Needs operator, Summary, Final answer, and Latest assistant text. Console builds a default `— activity —` document from Activity events and keeps `— raw —` as the debug fallback. Activity command/tool blocks render as Pi-themed framed blocks with status labels, `$` command rows, nested output snippets, footer metadata (`took … · raw:r`), and explicit elision (`… +N lines hidden`). Git-style `+` / `-` / `±` output keeps its text markers and uses success/danger/accent theme roles; final answers render parsed fields such as Headline, Risks, and Next.
