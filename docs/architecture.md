...
On `session_start`, the extension calls `ctx.isProjectTrusted()`. If the project is trusted, `loadActiveTeamConfig({ cwd, projectConfigTrusted: true })` may read the nearest ancestor `.pi/agent/agents-team.json`; if untrusted, project-local config is skipped and global/built-in config is used. The package requires Pi `>=0.85.1`. The Project Trust compatibility guard still treats a missing trust API as trusted so unexpected host shapes fail open rather than crashing. When Pi exposes `ctx.scopedModels`, the current canonical model references are added to the `delegate_task.model` description as bounded, advisory suggestions. An empty or absent list remains unrestricted, and role configuration or worker extensions may still expose worker-only models; the scope is never converted into validation. When a project file is loaded, prompt paths and scope roots resolve relative to the config file's project root, then the merged config is handed to `TeamManager`. That merged config is the active runtime authority for the session.
