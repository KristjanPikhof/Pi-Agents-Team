...
const DelegateTaskSchema = Type.Object({
	...
	cwd: Type.Optional(Type.String(), { description: "Working directory for the worker. Omit to use the current session cwd." }),
	contextHints: Type.Optional(Type.Array(Type.String()), { description: "Compact context bullets to pass into the worker, when needed." }),
	expectedOutput: Type.Optional(Type.String(), { description: "Describe the output contract the worker should return, when useful." }),
	pathScopeRoots: Type.Optional(Type.Array(Type.String()), { description: "Allowed path roots for scoped workers, especially write-capable profiles, when scoped." }),
	pathScopeAllowWrite: Type.Optional(Type.Boolean(), { description: "Whether the delegated path scope may be written to, when a path scope is set." }),
	skills: Type.Optional(Type.Array(Type.String()), { description: "List of installed Pi skill names to enable on the worker, when a specialized skill is needed. When set, Pi's skill discovery runs for this worker (normally disabled for worker-minimal launches) and the worker is told to load and apply the requested skills by name." }),
	model: Type.Optional(Type.String(), { description: DELEGATE_TASK_MODEL_DESCRIPTION }),
	reuseWorkerId: Type.Optional(Type.String(), { description: "Reuse an existing idle (or waiting_followup) worker's RPC session for this task instead of spawning a fresh process, or omit to launch a new worker. Use reuse when the next task is in scope of the previous role and roughly the same path scope — saves spawn cost and keeps warm role context. The worker's prior summary, finalAnswer, and lastTool are reset; a new taskId is allocated. Rejected if the target is running/starting/completed/aborted/error/exited (its RPC is already disposed); cancel + delegate fresh in that case. Check agent_status for `reusable: true` to find candidates." }),
}, { additionalProperties: false });

const WorkerLookupSchema = Type.Object({
	workerId: Type.Optional(Type.String(), { description: "Specific worker id, or omit to inspect all tracked workers." }),
}, { additionalProperties: false });
...
const WorkerMessageSchema = Type.Object({
	...
	delivery: Type.Optional(Type.Union([Type.Literal("auto"), Type.Literal("steer"), Type.Literal("follow_up")]), {
		description:
			'Delivery mode: "auto", "steer", or "follow_up". Omit for the default auto behavior. Applied only when the worker is running; idle/waiting_followup workers always receive the message as a fresh prompt that wakes the session and starts a new turn.',
	}),
}, { additionalProperties: false });

const PingAgentsSchema = Type.Object({
	workerIds: Type.Optional(Type.Array(Type.String()), { description: "Worker ids to ping, or omit to ping all workers." }),
	mode: Type.Optional(Type.Union([Type.Literal("passive"), Type.Literal("active")]), { description: 'Ping mode: "passive" or "active". Omit for passive. Active mode refreshes state and stats.' }),
}, { additionalProperties: false });
...
const WaitForAgentsSchema = Type.Object({
	workerIds: Type.Optional(Type.Array(Type.String()), { description: "Worker ids to wait on, or omit to wait on every tracked worker." }),
	timeoutMs: Type.Optional(Type.Number(), { description: "Maximum wait in milliseconds, or omit to use 300000 (5 min)." }),
	wakeOnRelay: Type.Optional(Type.Boolean(), { description: "Return early with reason=relay_raised when any target raises a new relay question. Omit for the default true behavior so the orchestrator can answer mid-flight without waiting for every worker to finish." }),
}, { additionalProperties: false });
