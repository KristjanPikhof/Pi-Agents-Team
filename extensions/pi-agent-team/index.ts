...
const DelegateTaskSchema = Type.Object({
	...
	reuseWorkerId: Type.Optional(Type.String({ description: "Reuse an existing idle (or waiting_followup) worker's RPC session for this task instead of spawning a fresh process, or omit to launch a new worker. Use reuse when the next task is in scope of the previous role and roughly the same path scope — saves spawn cost and keeps warm role context. The worker's prior summary, finalAnswer, and lastTool are reset; a new taskId is allocated. Rejected if the target is running/starting/completed/aborted/error/exited; cancel + delegate fresh in that case. Check agent_status for `reusable: true` to find candidates." })),
