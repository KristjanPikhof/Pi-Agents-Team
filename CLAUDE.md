...
Prune is not cancel. `cancelWorker` kills the RPC process and marks the entry `exited` but keeps it. `pruneTerminalWorkers` only removes already-terminal entries; never touches live processes. No auto-prune on terminal transition — operators want batch history until `/team-prune`.
