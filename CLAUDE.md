...
**Config schema is v2: roles are free-form, project replaces global.** `agents-team.json` (at `~/.pi/agent/` or `<cwd-ancestor>/.pi/agent/`) is the source of truth when present. Key rules:

- `version: 2` is the current schema. Unsupported versions (including v1 nested-permissions files) emit a `schema_version_mismatch` warning toast and fall back to built-in roles for that layer until the user runs `/team-init <scope> --force` (which backs up the old file).
- **Winning-layer-replaces-all.** When both global and project files exist, project wins *entirely* — global's roles are not merged in. This is a deliberate simplification from the earlier layered-narrowing model.
...
- **Prompt resolution**: `"default"`/omitted → packaged `prompts/agents/<name>.md` if the role name matches one of the seven built-ins, else the generic template at `prompts/agents/_generic-worker.md` with `{NAME}` / `{DESCRIPTION}` substituted. A path string that resolves to a readable file → that file. A string that doesn't resolve → stored as `promptInline` on the profile spec and served verbatim. A path that escapes the project root → hard error (security).
