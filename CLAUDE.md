...
**Config layering: global + project, enabled flag per layer.** `agents-team.json` lives at two locations and both are optional:
- **Global**: `~/.pi/agent/agents-team.json` — user-level defaults.
- **Project**: `<cwd or ancestor>/.pi/agent/agents-team.json` — nearest ancestor wins.

`loadActiveTeamConfig` probes both, layers global first then project (field-by-field; project cannot broaden past global, which cannot broaden past built-ins), and resolves `enabled` by precedence: project > global > default `true`. When resolved `enabled` is false, session_start shows a one-line disabled toast pointing at the source file, the "Pi Agents Team loaded" banner is hidden, tools refuse with `getDisabledMessage`, `before_agent_start` returns the original systemPrompt untouched, and `applyUi` clears status + widget. Commands still register and toggle commands still work so operators can re-enable without editing JSON. `PI_AGENT_TEAM_GLOBAL_CONFIG_PATH` env var overrides the global probe (used by extension tests for isolation; `"none"`/`""`/`"null"` skips it entirely). When adding fields to `LoadedTeamProjectConfig`, preserve the `layers[]`, `enabled`, and `enabledSource` semantics — they drive the toast source line.

**Team profiles and Pi skills are different axes.** `delegate_task.profileName` must be one of the seven packaged profiles (`explorer`, `librarian`, `oracle`, `designer`, `fixer`, `reviewer`, `observer`). The Pi `[Skills]` list (e.g. `writer`, `frontend-design`, `architecting-systems`) is **not** a profile list — those are host-level capabilities the worker's Pi session can load via the Skill tool. Pass them through the optional `delegate_task.skills: string[]` array, which `buildWorkerTaskPrompt` renders as an explicit "invoke these skills via the Skill tool" block in the worker prompt. Without this distinction the orchestrator conflates the two and fails with `Unknown team profile: writer`. Keep the orchestrator prompt's "Profiles vs skills" section (`prompts/orchestrator.md`) and the matching paragraph in `docs/prompting.md` in sync with the schema.
