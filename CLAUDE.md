...
Reuse is launch-strict. `delegate_task.reuseWorkerId` re-prompts existing idle/waiting_followup worker's RPC instead of spawning. Process-launch flags (`model`, `tools`, `cwd`, `systemPromptPath`, `extensionMode`, `thinkingLevel`, `allowSkills`) baked at spawn. `WorkerManager` snapshots them on `launchWorker` (`record.launchSnapshot`); `TeamManager.reuseWorkerForTask` recomputes via `applyLaunchPolicy` and rejects per-field on mismatch. Cross-profile reuse rejected. Reuse resets per-task state (`textBuffer`, `finalAnswer`, `lastTool`, `relayQuestions`, `lastSummary`, `error`, `assistantChunks`) before re-prompting.

`closing` flag is load-bearing. `WorkerRuntimeRecord.closing` lets `closeWorker` distinguish operator disposal from crash. Without it, natural `worker_exit` from `handle.dispose()` maps `SIGTERM` → `aborted`. `closeWorker` sets `closing = true`, then disposes; `worker_exit` branch checks flag and forces `exited`. Don't reuse for other paths; cancel does NOT set it.
...
Visible-width + ANSI handling in TUI code. Widget and overlay use pi-tui `visibleWidth` / `truncateToWidth` (both ANSI-aware: strip CSI before measuring). Never raw `.length` / `.slice`. Overlay's `sanitizeText` normalizes `\t` → 4 spaces and strips control bytes EXCEPT `\x1b`. Widening the strip regex to include `0x1b` silently strips theme.ts colors. Pi-tui validates rendered line widths against terminal columns and crashes the session if exceeded; that's why `\t` must be normalized (terminal expands tabs to 8 cols though `visibleWidth` measures as 1).

Overlay row budget must match `maxHeight`. `TEAM_DASHBOARD_OVERLAY_OPTIONS.maxHeight = "90%"` and `OVERLAY_HEIGHT_PCT = 0.9` are paired. `computeOverlayRows(termRows) = floor(termRows * OVERLAY_HEIGHT_PCT)`. If render returns more rows than the panel's clipped rectangle, pi-tui truncates the tail and the bottom frame + footer disappear. Bumping one constant requires bumping the other and updating the row-budget regression test in `tests/ui/overlay.test.ts`.
...
Overlay `dispose()` is exposed and idempotent. Component returns `{ render, invalidate, handleInput, dispose }`. `q`/`esc` paths call `dispose()` then `done()`. If pi-tui closes overlay outside our `done()`, host should call `dispose()` to unsubscribe `onAssistantChunk`. Without it the listener leaks across open/close cycles.
