...
export function buildWorkerTaskPrompt(task: DelegatedTaskInput): string {
	...
	return [
		...
		skills.length > 0
			? `Requested Pi skills for this task:\n- ${skills.join("\n- ")}\n\nPi has loaded available skill metadata into your system prompt context. Use read to load each relevant requested skill, or bash when read is unavailable. Apply the skill before producing your \`<final_answer>\`. Do not output skill commands as the deliverable. If a requested skill is not available in this Pi session, note it in the final answer and proceed without it.`
			: undefined,
